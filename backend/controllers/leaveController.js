...
const applyLeave = async (req, res) => {
  try {
    ...
    runBackgroundTask("Leave application side effects", async () => {
      await Promise.all([
        sendEmail({
          to: employee.email,
      ...
      if (employee.reportingOfficerId) {
        ...
        const reportingOfficer = await getUserProfile(employee.reportingOfficerId);
    ...
    const nextOfficer =
      stage === "reporting"
        ...
        : stage === "reviewing"
          ...
          : null;
    runBackgroundTask("Leave review side effects", async () => {
      await Promise.all([
        updateDashboardStats(leave.employeeId),
        notify({
      ...
      if (stage === "approving") {
        ...
        await sendEmail({
          to: employee.email,
          subject: decision === "approved" ? "Leave Approved" : "Leave Rejected",
...
const cancelMyLeave = async (req, res) => {
  try {
    ...
    runBackgroundTask("Leave cancellation side effects", async () => {
      const employee = await getUserProfile(req.user.uid);
